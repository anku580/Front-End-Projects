...
var resizePizzas = function(size) {
    ...
    function changePizzaSizes(size) {
        switch (size) {
            case "1":
                newWidth = 25; // small width is given user whenever scrolls the bar the size of pizza becomes this.
                break;
            case "2":
                newWidth = 33.3; // medium width is given user whenever scrolls the bar the size of pizza becomes this.
                break;
            case "3":
                newWidth = 50; // large width is given user whenever scrolls the bar the size of pizza becomes this.
                break;
            default:
                console.log("bug in sizeSwitcher");
        }
        var randy = document.querySelectorAll(".randomPizzaContainer"); // it select the tandompizzacontainer , i have stored it in variable so that whenever i have to access this i can easily use a variable name to acesss this property
        for (var i = 0; i < randy.length; i++) { //loop is created
            randy[i].style.width = newWidth + "%"; //size of the pizza get changed with the newWidth variable and % is attached to it.
        }
    }
...
function updatePositions() {
    ...
    for (var i = 0; i < items.length; i++) {
       phase.push(Math.sin((document.body.scrollTop / 1250 ) + (i % 5))); //  this changes the layout so it should comes before the syle so for loop is first active for this
    }

    for (var i = 0; i < items.length; i++) {
        items[i].style.left = items[i].basicLeft + 100 * phase[i] + 'px'; // after the layout changes we can add the style we wants to choose
    }
